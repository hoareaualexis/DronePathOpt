...
def poly_def(lis):
    ...
    arcs = [(p0, p1), (p1, p2), (p2, p3), (p3, p4), (p4, p5), (p5, p6),
            ...
            (p11, p0)]  ## Liste des cotes du polygone definis sous formes d'arcs
...
def concave_points(lis):
    ...
    return concave_points, concave_index


def intersection_points(lis):
    conc_index, _ = concave_points(lis)
    ...
    for j in range(len(conc_index)):
        ...
        if len(show_inter) > 2:
            ...
            for i in range(len(show_inter) - 1):
                dist = liste[index].distance(show_inter[i])
                if short_dist > dist:
                    short_dist = dist
                    near = i
...
def arcs_add(interpoint, index, arcs, liste):
    nbre_arc = len(arcs)
    indic = 0
    ...
    for i in range(nbre_arc):
        ...
        for j in range(1):
            ...
            dist = interpoint.distance(segment)
            if dist == 0:
                if (interpoint == arcs[i][j]) or (interpoint == arcs[i][j + 1]):
                    arcs.insert(index, (liste[index], interpoint))
                else:
                    arcs.insert(index, (liste[index], interpoint))
                    arcs.insert(i + 1, (arcs[i][j], interpoint))
                    arcs.insert(i + 2, (interpoint, arcs[i][j + 1]))
                nbre_arc = len(arcs)
                indic = 1
        if indic == 1: break
